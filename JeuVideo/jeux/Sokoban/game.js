...
function preload ()
{
    this.load.image('nouveau', './jeux/Sokoban/assets/nouveau.png')
    this.load.spritesheet('tiles', './jeux/Sokoban/assets/sokoban_tilesheet.png',{ 
        frameWidth: 64, 
        startFrame: 0 }
        );
...
function update ()
{
    ...
    let i=verifierTuilesBoxBleues()
      ...
      if (mouvement<=30 ) 
      {
        if(i===1)
        {
           Game.call(this, 'Bravo Level complet vous avez obtenu ',score); 
...
function Game(message, mouvement) 
{
  ...
  var gameOverText = this.add.text(game.config.width / 2, game.config.height / 2, message + ' ' + mouvement + ' score!',
  {
